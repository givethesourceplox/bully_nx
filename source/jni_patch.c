...
static const JniGamepadButtonMap g_jni_gamepad_buttons[] = {
    ...
    // Keep L3/R3 on the callback path only; the legacy bitmask path collides
    // with the Android D-pad semantics in this build.
    {HidNpadButton_StickL, 0, 12, "l3"},
    {HidNpadButton_StickR, 0, 13, "r3"},
    // Physical D-pad is owned by the direct LIB_GamepadState hook, not by the
...
static void *async_file_worker_main(void *arg)
{
  ...
  (void)arg;
  last_ns = armGetSystemTick() * 1000000000ULL / armGetSystemTickFreq();
  ...
  while (1)
  {
    ...
    delta = (double)(now_ns - last_ns) / 1000000000.0;
    last_ns = now_ns;
    if (delta <= 0.0 || delta > 0.1)
      delta = 0.002;

    if (*g_async_first_file_ptr)
...
void jni_start(void)
{
  ...
  while (1)
  {
    ...
    if (saved_AND_FileUpdated && asyncHead)
      saved_AND_FileUpdated((double)delta);
    saved_implOnDrawFrame(fake_env, NULL, delta);
    frame++;
    svcSleepThread(16000000LL); // 16ms ~ 60Hz
  }
}
